...
func (a *accountImplement) TopUp(c *gin.Context) {
	...
	err = a.db.Model(&model.Account{}).Where("id = ?", accountID).
...
func (a *accountImplement) Balance(c *gin.Context) {
	...
	err := a.db.Select("balance").Where("id = ?", accountID).First(&account).Error
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not load balance details"})
		return
	}
